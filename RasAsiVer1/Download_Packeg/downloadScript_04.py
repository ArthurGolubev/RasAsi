...
def download():
    ...
    print(os.path.normpath(rf'{keypath1}\{listdir0[variable2]}'))
    with open(os.path.normpath(rf'{keypath1}\{listdir0[variable2]}'), 'r') as file1:
        ...
        logFile = open(os.path.normpath(rf'{keypath1}\logFile{createlogName}.txt'), 'a')
        ...
        for i in range(links_number - 1):                                                                               #считает i с 0, следовательно цифру количества ссылок (которая считается с 1, а не с 0) нужно убавить на 1
            ...
            try:                                                                                                        #Если такой папки нет - создаёт (нужно для первой ииерации при каждом новом названии директории)
                os.makedirs(os.path.normpath(f'{keypath1}\{list1[3]}\{list1[4]}\{list1[5]}'))
                os.chdir(os.path.normpath(f'{keypath1}\{list1[3]}\{list1[4]}\{list1[5]}'))
        ...
        with open(os.path.normpath(fr'{keypath1}\logFile{createlogName}.txt'), 'a') as logFile:                                #Общий лог за скачивание
            logFile.write('\n\n')
            os.chdir(os.path.normpath(f'{keypath1}\{list1[3]}'))
            ...
            for i2 in listdir1:                                                                                         #Если скачивалось в 2 и более директории
                sizeDir = 0
                os.chdir(os.path.normpath(fr'{keypath1}\{list1[3]}\{i2}'))
                listdir2 = os.listdir()
                for i3 in listdir2:
                    os.chdir(os.path.normpath(fr'{keypath1}\{list1[3]}\{i2}\{i3}'))
    ...
    if int(option1):
        os.startfile(os.path.normpath(fr'{keypath1}\logFile{createlogName}.txt'))
    else:
        return 0


def downloadFuc(list1, string1, i, links_number, createlogName, keypath1):
    with open(os.path.normpath(fr'{keypath1}\logFile{createlogName}.txt'), 'a') as logFile:
