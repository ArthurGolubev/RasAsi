    ...
    def show_places(self, upass):
        ...
        for location in locations:
            marker_text = fr'Coordinate - {location[1]}, {location[2]}\n' \
                ...
                fr',3964m/data=!3m1!1e3?hl=ru", target="_blank">GoogleEarth<\a> 😉👌\n' \
                fr'{location[3]}'
            folium.Marker(location=(location[1], location[2]), popup=marker_text, icon=folium.Icon(icon='cloud', color='green')).add_to(m)
            folium.Circle(location=(location[1], location[2]), radius=100000, popup=marker_text, icon=folium.Icon(icon='cloud', color='green')).add_to(m)
